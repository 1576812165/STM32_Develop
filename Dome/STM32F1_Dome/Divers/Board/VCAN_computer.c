...
void uart_send_data(uint8_t *buf, uint16_t len,USART_TypeDef *USART)
{
	...
	while((USART_GetFlagStatus(USART, USART_FLAG_TC) == RESET));//等待串口发送完毕防止丢失第一个字节 
	for(i=0;i<len;i++)
	{  
		USART_SendData(USART,buf[i]);      		//写DR,串口1将发送数据
		while((USART_GetFlagStatus(USART, USART_FLAG_TC) == RESET));//等待串口发送完毕
	}	
}
...
#define CMD_IMG     1
    ...
    uart_send_data(cmdf, sizeof(cmdf),USART);    //先发送命令

    uart_send_data((uint8_t *)imgaddr, imgsize,USART); //再发送图像

    uart_send_data(cmdr, sizeof(cmdr),USART);    //先发送命令
}
...
#define CMD_CCD     2
    ...
    uart_send_data((uint8_t *)ccdaddr, ccdsize,USART); //再发送图像

    uart_send_data(cmdr, sizeof(cmdr),USART);    //再发送命令
}
...
#define CMD_WARE     3
    ...
    uart_send_data(cmdf, sizeof(cmdf),USART);    //先发送前命令
    uart_send_data((uint8_t *)wareaddr, waresize,USART);    //发送数据
    uart_send_data(cmdr, sizeof(cmdr),USART);    //发送后命令
}
